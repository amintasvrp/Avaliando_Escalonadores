...
scheduler(void)
{
  ...
  for(;;){
    ...
    int ticketsPassed = 0, totalTickets = 0;

    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state != RUNNABLE)
        continue;

      totalTickets = totalTickets + 32-p->prio;  
    }
...
setprio(int pid,int prio)
{
  ...
  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->pid == pid && prio >= 0 && prio < 32){
      cprintf("setting process with pid %d, with %d priority\n", p->pid, prio);
      p->prio = prio;
      if(p->state == SLEEPING){
        p->state = RUNNABLE;
      }
